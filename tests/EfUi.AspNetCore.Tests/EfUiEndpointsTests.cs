...
public class EfUiEndpointsTests : IClassFixture<EfUiApplicationFactory>
{
    ...
    public async Task Get_table_enhancement_assets_expose_tabulator_bootstrap_shell()
    {
        ...
        script.Should().Contain("headerFilterFunc:");
        script.Should().Contain("matchesHeaderFilter(column, rowValue, headerValue)");
        script.Should().Contain("compareCellValues(left, right)");
        script.Should().Contain("renderStatus");
        script.Should().Contain("dataLoaderLoading");
        script.Should().NotContain("fetch(");
        script.Should().NotContain("dataUrl");
        script.Should().NotContain("popstate");
        script.Should().NotContain("setHeaderFilterValue");
        script.Should().NotContain("setSort");
        script.Should().NotContain("dataFiltered");
        script.Should().NotContain("readActiveHeaderFilters(table)");
        script.Should().NotContain("table.getHeaderFilters()");
        script.Should().NotContain("headerClick: isActionsColumn || column.headerSort === false");
        ...
        script.Should().NotContain("getFilterOperator(field, value)");
        script.Should().NotContain("column.activeFilterOperator");
        script.Should().NotContain("params.set('sort.' + sortIndex + '.field'");
        script.Should().NotContain("params.set('sort.' + sortIndex + '.dir'");
        script.Should().NotContain("filterIndex");
        script.Should().NotContain("sortIndex");
        ...
        script.Should().NotContain("headerClick: undefined");
        script.Should().NotContain("title += ' ↑'");
        script.Should().NotContain("efui-query-form");
        script.Should().NotContain("await applyPayload(payload);");
        script.Should().NotContain("replaceBrowserUrl(listUrl, params);");
        script.Should().Contain("Unable to load table.");
        script.Should().NotContain("await applyPayload(config);");
    ...
    public async Task Get_entity_page_renders_active_query_state_from_url_in_compact_status_area()
    {
        ...
        emailColumn.GetProperty("activeFilterOperator").ValueKind.Should().Be(JsonValueKind.Null);
    ...
    public void Readme_describes_simple_and_chinook_breadcrumb_and_column_filter_list_behavior()
    {
        ...
        readme.Should().Contain("Package details: [src/EfUi.AspNetCore/README.md](src/EfUi.AspNetCore/README.md)");
